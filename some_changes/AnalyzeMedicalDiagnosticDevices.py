...
class AnalyzeMedicalDiagnosticDevices:
    ...
    def create_pivot_table(self) -> pd.DataFrame:
        """Создание сводной таблицы с клиниками и оборудованию.

        Returns:
            df: DataFrame, сводная таблица с датами последней калибровки и количеством проблем в клиниках.
        """

        df = self.__dataframe_medical_devices
        clinic_prolems = df.groupby('clinic_name')['issues_reported_12mo'].sum().to_dict()
        df['count_of_problems_in_clinic'] = df['clinic_name'].map(clinic_prolems)
        df['calibration_in_future'] = (self.last_calibration_future())['last_calibration_date']
        df['calibration_in_past'] = (self.last_calibration_past())['last_calibration_date']
        df['calibration_date_is_incorrect'] = (self.last_calibration_incorrect())['last_calibration_date']
        pivot = df.pivot_table(values = ['calibration_in_future', 'calibration_in_past', 'calibration_date_is_incorrect'], index = ['clinic_name' , 'device_id', 'count_of_problems_in_clinic'])
